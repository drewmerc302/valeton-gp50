...
updated: 2026-07-13 (seed) | branch: mvp-converter | phase: 0 | acceptance: 0/7

## Now / next
- Seeded by setup. Next tick: Architect pass → refine this DAG, then start T0 scaffold.

## Backlog (DAG)
- [ ] T0  scaffold FastAPI app under `app/`, `.venv-app`, pytest+playwright harness, run.sh   (deps: —)        [todo]
- [ ] T1  engine API: `app/engine.py` wrapping the 2-venv convert as a job w/ progress callbacks + 0.5.x|0.7.0 toggle   (deps: T0)  [todo]
